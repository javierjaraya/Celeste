...
class Detalle_compraDAO {
    ...
    public function findAllByIDProducto($idProducto) {
        $this->conexion->conectar();
        $query = "SELECT dc.idDetalle, dc.idCompra, dc.idProducto, dc.precio, dc.cantidad, p.nombreProducto FROM detalle_compra as dc join producto as p on p.idProducto = dc.idProducto where p.idProducto = " . $idProducto;
        $result = $this->conexion->ejecutar($query);
        $i = 0;
        $detalle_compras = array();
        while ($fila = $result->fetch_row()) {
